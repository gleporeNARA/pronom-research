...
proc iface_win_browse_fs {place} {
	...
	if { [file pathtype $place] == "relative" } {
		...
		-message "O caminho fornecido não pode ser relativo.\nListando o caminho pré-definido:\n$place"
	}
    }
	...
	if {! [file exists $other_place]} {
	    tk_messageBox -icon error -type ok -parent . -title $igl_prog_name \
		-message "O caminho fornecido não foi encontrado\n$place\no caminho prédefinido também não:\n$other_place\n\nFechando janela."
	    destroy .brw
	}

	# alert user
	tk_messageBox -icon error -type ok -parent . -title $igl_prog_name \
	    -message "Não foi encontrado:\n$place\nListando a alternativa prédefinida:\n$other_place"
    ...
    wm title .brw "Escolher"
    wm iconname .brw [join [list $igl_prog_name " - Escolher"] ""]
    ...
    label .brw.inform -text "Clique em \"Aceitar\" para escolher:" -anchor w
    ...
    button .brw.fr_bts.bt_sel -text "Aceitar" -command {
	# trigger event that handle ACCEPT
	set id [winfo id .brw.files.lst]
	event generate $id <<Accept>>
    }
    button .brw.fr_bts.bt_up -text "Recuar \[..\]" -command { 
    ...
    button .brw.bt_cancel -text "Cancelar" -command {destroy .brw}
	...
	if {[file exists $place]} {
		...
		tk_messageBox -icon warning -type ok -parent .brw -title $igl_prog_name \
		    -message "Impossível listar o conteúdo do diretório (pode ser que esteja vazio).\n"
	    }
	    # for each entry
...
proc iface_create_cell {lin} {
	...
	button .tab_ctls.cells.b_new_line -text "ou . . ." -command "ih_create_cell -1"
	...
	button $nome -text "e mais . . ." -command "ih_create_cell $lin"
...
proc iface_create_grid {} {
  ...
  grid .tab_ctls.scroll_v -in .tab_ctls -row 0 -column 1 -pady 4 -sticky ns
    ...
    .tab_sel.s0 configure -background $backc -text "O que pesquisar" -font {Courier 12 bold}
    .tab_sel.s1 configure -background grey85 -text "Onde" -font {Courier 14}
    .tab_sel.s2 configure -background grey85 -text "Como" -font {Courier 14}
}
...
proc iface_create_where_lst {} {
    ...
    button .tab_ctls.f_wbs.b_w0 -text "Adicionar..." -command "ih_add_fs_entry"
    button .tab_ctls.f_wbs.b_w1 -text "Apagar..." -command "ih_remove_fs_entries"
    ...
    set backc [lindex [. configure -background] end]
    .tab_sel.s0 configure -background grey85 -text "O que" -font {Courier 14}
    .tab_sel.s1 configure -background $backc -text "Onde pesquisar" -font {Courier 12 bold}
    .tab_sel.s2 configure -background grey85 -text "Como" -font {Courier 14}
}
...
proc iface_create_options_bar {} {
  ...
  checkbutton .tab_ctls.left_bar.w0 -text "converter acentos do HTML" -variable igl_c_translate -relief flat
  checkbutton .tab_ctls.left_bar.w1 -text "diferenciar maiúsculas de minúsculas" -variable igl_c_case -relief flat
  checkbutton .tab_ctls.left_bar.w2 -text "procurar dentro de subdiretórios" -variable igl_c_subdirs -relief flat
  checkbutton .tab_ctls.left_bar.w3 -text "preprocessar arquivo, se possível" -variable igl_c_preprocess -relief flat
  checkbutton .tab_ctls.left_bar.w4 -text "procurar apenas conteúdos textuais" -variable igl_c_text_only -relief flat
  ...
  label .tab_ctls.left_bar.l_cm -text "Comando de pesquisa (e opções permanentes):" -anchor w
  entry .tab_ctls.left_bar.e_cm -textvariable igl_e_search_command
  label .tab_ctls.left_bar.l_av -text "Evitar os arquivos com os seguintes sufixos:" -anchor w
  entry .tab_ctls.left_bar.e_av -textvariable igl_e_avoid_files
  label .tab_ctls.left_bar.l_tp -text "Diretório temporário:" -anchor w
  entry .tab_ctls.left_bar.e_tp -textvariable igl_e_temp_dir
  label .tab_ctls.left_bar.l_as -text "Associações entre arquivos e preprocessadores:" -anchor w
  entry .tab_ctls.left_bar.e_as -textvariable igl_e_preprocess_assocs
  label .tab_ctls.left_bar.l_ac -text "(%n será substituído pelo nome do arquivo a ser preprocessado)" -anchor w
   pack	.tab_ctls.left_bar.l_cm .tab_ctls.left_bar.e_cm \
	...
	.tab_ctls.left_bar.l_ac -anchor w -pady 2 -fill x -anchor w

  label .tab_ctls.right_bar.l_co -text "Comparar com o..." -height 1 -anchor s
  radiobutton .tab_ctls.right_bar.r_co0 -text "nome" -relief flat \
                            -variable igl_r_where -value 1
  radiobutton .tab_ctls.right_bar.r_co1 -text "conteúdo" -relief flat \
                            -variable igl_r_where -value 2
  radiobutton .tab_ctls.right_bar.r_co2 -text "nome e o conteúdo" -relief flat \
                            -variable igl_r_where -value 3
  label .tab_ctls.right_bar.l_co2 -text "... dos arquivos e pastas." -height 1 -anchor s
  pack .tab_ctls.right_bar.l_co .tab_ctls.right_bar.r_co0 \
    ...
    .tab_sel.s0 configure -background grey85 -text "O que" -font {Courier 14}
    .tab_sel.s1 configure -background grey85 -text "Onde" -font {Courier 14}
    .tab_sel.s2 configure -background $backc -text "Como pesquisar" -font {Courier 12 bold}
}
...
proc iface_create_result {} {
	...
	.tab_sel.s0 configure -foreground grey20 -background grey85 -text "O que" -font {Courier 12}
        .tab_sel.s1 configure -foreground grey20 -background grey85 -text "Onde" -font {Courier 12}
        .tab_sel.s2 configure -foreground grey20 -background grey85 -text "Como" -font {Courier 12}
        frame .tab_ctls -relief flat
        pack .tab_ctls -side top -fill both -expand yes

	button .tab_ctls.b_search -text "Pesquisar..." -command "ih_search"
...
proc iface_update_var_menu {new_view} {
  ...
  if {$new_view == 0} {
    .main_menu.edit add command -label "Apagar células vazias" \
      -underline 0 -command "ih_remove_empty_cells $igl_prog_name"
  } else {
    .main_menu.edit add command -label "Apagar..." -underline 0 \
      -command "ih_result_select_delete"
    .main_menu.edit add separator
    .main_menu.edit add command -label "Selecionar Tudo" -underline 11 \
      -command "ih_result_select_all"
    .main_menu.edit add command -label "Selecionar Nada" -underline 11 \
      -command "ih_result_unselect_all"
    .main_menu.edit add command -label "Inverter Seleção" -underline 0 \
...
proc iface_win_create_main {} {
  ...
  .main_menu.file.reutilizate add command -label "Todos" \
    -underline 0 -command "ih_save_defaults -1"
  .main_menu.file.reutilizate add separator
  .main_menu.file.reutilizate add command -label "O que" \
    -underline 0 -command "ih_save_defaults 0"
  .main_menu.file.reutilizate add command -label "Onde" \
    -underline 1 -command "ih_save_defaults 1"
  .main_menu.file.reutilizate add command -label "Como" \
  ...
  .main_menu.file add command -label "Novo..." -underline 0 -command "ih_new_file $igl_prog_name"
  .main_menu.file add command -label "Abrir..." -underline 0 -command "ih_open_file $igl_prog_name"
  .main_menu.file add command -label "Salvar" -underline 0 -command "ih_save_file"
  #.main_menu.file add command -label "Salvar Como..." -underline 7 -command "ih_save_as_file"
  .main_menu.file add separator
  .main_menu.file add command -label "Exportar Resultado..." -underline 0 \
      -command "ih_result_export"
  .main_menu.file add separator
  .main_menu.file add cascade -menu .main_menu.file.reutilizate -label "Reutilizar" -underline 2
  .main_menu.file add separator
  .main_menu.file add command -label "Sair..." -underline 3 -command "ih_end_app $igl_prog_name"

  # this menu will change acording application state
  .main_menu add cascade -menu .main_menu.edit -label "(Editar)"

  # view submenu
  menu .main_menu.view -tearoff 0
  .main_menu add cascade -menu .main_menu.view -label "Visualizar" \
    -underline 0
  .main_menu.view add radiobutton -label "Planejamento" \
    -underline 0 -value 0 -variable igl_active_view -command \
    "iface_update_ctls 1 0 -1"
  .main_menu.view add radiobutton -label "Resultado da Pesquisa" \
    -underline 0 -value 1 -variable igl_active_view -command \
    "iface_update_ctls 1 1 -1"

  # help submenu
  menu .main_menu.help -tearoff 0
  .main_menu add cascade -menu .main_menu.help -label "Ajuda" \
    -underline 1
  .main_menu.help add command -label "Sobre..." \
  ...
  label .tab_sel.s0 -text "(Planilha)" -relief flat -pady 1
  label .tab_sel.s1 -text "(Local)" -relief flat -pady 1
  label .tab_sel.s2 -text "(Opções)" -relief flat -pady 1
...
proc iface_win_add_fs_entry {edit_entry} {
    ...
    # define basic spects
    wm title .ew "Adicionar Local Onde Pesquisar"
    wm iconname .ew [join [list $igl_prog_name " - Adicionar Local"] ""]
    wm minsize .ew 300 50
    wm resizable .ew 1 0

    # create graphical interface

    label .ew.cab -text "Digite o local (clique em \"Escolher\" para selecioná-lo de uma listagem):" -anchor w
    entry .ew.e
    button .ew.b_sel -text "Escolher..." -command {
	# get the entry name
	set res ""
    ...
    if {$edit_entry == 0} {
      button .ew.rod.b_add -text "Adicionar" -command {
	  .tab_ctls.f_l.l_f insert end [.ew.e get]
	  destroy .ew
      }
    } else {
    ...
    button .ew.rod.b_canc -text "Cancelar" -command "destroy .ew"
...
proc iface_remove_fs_entries {} {
    ...
    if {$sel_indexes == ""} {
	tk_messageBox -icon error -type ok -parent . -title $igl_prog_name \
	    -message "Antes de apagar, é necessário selecionar os ítens da lista, clicando sobre os mesmos."
	return
    } else {
	...
	set confirm [tk_messageBox -icon question -type yesno -parent . -title $igl_prog_name \
			 -message "Deseja apagar $num elemento(s) da lista?"]
...
proc iface_win_init_search {} {
	...
	tk_messageBox -icon error -type ok -parent . -title $igl_prog_name \
		 -message \
		 "Para pesquisar por um texto preencha a planilha com o texto."
	iface_update_ctls 0 0 0
	 return
    }
    ...
    # define basic spects
    wm title .epw "Iniciar Pesquisa"
    wm iconname .epw [join [list $igl_prog_name " - Iniciar Pesquisa"] ""]
    wm minsize .epw 450 100
    wm resizable .epw 1 0

    # create graphical interface

    label .epw.cab -text "A pesquisa será executada pelo seguinte comando:" -anchor w
    pack .epw.cab -side top -fill both -pady 2 -anchor w

    frame .epw.rod
    pack .epw.rod -side bottom -pady 1 -fill x

    button .epw.rod.b_exec -text "Iniciar" -command {
	# to start search
	set exec_line [.epw.params get]
	destroy .epw
	iface_win_make_search $exec_line
    }
    button .epw.rod.b_canc -text "Cancelar" -command "destroy .epw"
...
proc iface_win_make_search {exec_cmd} {
    ...
    # define basic spects
    wm title .sw "Pesquisando"
    wm iconname .sw [join [list $igl_prog_name " - Pesquisando"] ""]
    ...
    button .sw.b_canc -text "Parar" -command {
	iface_win_end_search .sw
    }
    pack .sw.b_canc -fill both -side bottom

    label .sw.cab -text "Executando a pesquisa com o comando:" -anchor w
    ...
    label .sw.midt -text "Resultados parciais:" -anchor w
...
proc iface_win_end_search {out_win} {
    ...
    wm title $out_win "Resultado da Pesquisa"
    wm iconname $out_win [join [list $igl_prog_name " - Pesquisa Concluída"] ""]

    $out_win.b_canc configure -text "Fechar"

    # sound warning about search end
    bell

    tk_messageBox -icon info -type ok -title $igl_prog_name \
		     -message "Pesquisa terminada.\n\nOs ítens inclusos na lista dos resultados estarão selecionados."
...
proc iface_result_select_delete {} {
    ...
    set num [llength $sel_indexes]
    set confirm [tk_messageBox -icon question -type yesno -parent . -title $igl_prog_name \
		     -message "Deseja apagar $num linha(s) da lista?"]
...
proc iface_result_export {} {
  ...
  if { $num == 0 } {
	  ...
	  -message \
	  "Esta pesquisa não obteve resultados, a listagem que será arquivada estará vazia.\n\nÉ aconselhável realizar uma pesquisa que obtenha resultados antes de exportá-la."
  }
...
proc iface_win_help_about {} {
    ...
    # define basic spects
    wm title .aw "Sobre"
    wm iconname .aw [join [list $igl_prog_name " - Sobre"] ""]
    ...
    label .aw.t3 -text "Licensa: GNU-GPL 2"
    label .aw.t4 -text "E-mail: lebe@geocities.com"
    label .aw.t5 -text "Este programa foi projetado para auxiliar em pesquisas textuais por vários arquivos."
    label .aw.t6 -text " "
    button .aw.b -text "Fechar" -command {destroy .aw} -underline 0
...
proc ih_end_app {msg_title} {
    ...
    set confirm [tk_messageBox -icon question -type yesno -parent . -title $msg_title \
		     -message "Confirma o encerramento dessa sessão de trabalho?"]
...
proc ih_open_file {msg_title} {
  ...
  # confirm
  set confirm [tk_messageBox -icon question -type yesno -parent . -title $msg_title \
		     -message "Confirma a abertura de um arquivo?\n\nOs dados do trabalho atual serão substituídos pelos do arquivo."]

  # if is to replace
  if {$confirm == "yes"} {

      set file_types { {"arquivo de pesquisa" {.flz} }
	  {"Todos"  {*}}}
...
proc ih_save_defaults {what_save} {
	...
	tk_messageBox -icon info -type ok -parent . -title $igl_prog_name \
		     -message "Não foi possível gravar o arquivo de configuração."
    }
}
...
proc ih_new_file {msg_title} {
    ...
    # confirm
    set confirm [tk_messageBox -icon question -type yesno -parent . -title $msg_title \
		     -message "Confirma o início de um novo trabalho?\n\nOs dados do trabalho atual serão apagados."]
...
proc ih_remove_empty_cells {msg_title} {
	...
	tk_messageBox -icon info -type ok -parent . -title $msg_title \
	    -message "Somente é útil solicitar a remoção das células vazias se a planilha estiver em exibição. Caso contrário, as células vazias serão removidas automaticamente."
    } else {
...
proc ih_insert_cell {msg_title} {
    ...
    if {$igl_active_tab != 0} {
	tk_messageBox -icon info -type ok -parent . -title $msg_title \
	    -message "Somente podem ser incluídas novas células caso a planilha esteja em exibição."
    } else {
...
proc app_make_search {output_win output_widget exec_cmd} {
	...
	set err_msg "Não foi possível executar a pesquisa.\nA causa da falha pode ser identificada pela mensagem abaixo.\n\nMensagem:\n$out_msg"
	tk_messageBox -icon error -type ok -parent . -title "Erro" \
	    -message $err_msg
	return 0
    }
...
proc app_save_file {fname} {
    ...
    if {$chann == ""} {
	set err_msg "Não foi possível salvar o arquivo. A causa da falha pode ser identificada pela mensagem abaixo.\n\nMensagem:\n$out_msg"
	tk_messageBox -icon error -type ok -parent . -title "Erro" \
	    -message $err_msg
	return 0
    }
...
proc app_read_file {fname} {
    ...
    if {$chann == ""} {
	set err_msg "Não foi possível ler o arquivo.\nA causa da falha pode ser identificada pela mensagem abaixo.\n\nMensagem:\n$out_msg"
	tk_messageBox -icon error -type ok -parent . -title "Erro" \
	    -message $err_msg
	return 0
    }
    ...
    set fvers [gets $chann]
    if {$fvers != $vers} {
	set err_msg "Não foi possível ler o arquivo.\nO arquivo é incompatível com o tipo suportado por este programa."
	tk_messageBox -icon error -type ok -parent . -title "Erro" \
	    -message $err_msg
        close $chann
        return 0
...
proc app_export_result {file_name data_path data_name prog_name} {
    ...
    if {$chann == ""} {
	set err_msg "Não foi possível exportar o resultado para o arquivo. A causa da falha pode ser identificada pela mensagem abaixo.\n\nMensagem:\n$out_msg"
	tk_messageBox -icon error -type ok -parent . -title "Erro - $prog_name" \
	    -message $err_msg
    }
    ...
    if {$export_html == 1} {
	puts $chann "<html>\n"
	puts $chann "<HEAD><TITLE>Resultado da Pesquisa</TITLE></HEAD>\n"
	puts $chann "<body>\n"
    }
    ...
    puts $chann "Resultado da pesquisa executada pelo programa $prog_name."
    if {$export_html == 1} {
	puts $chann "</i><p>\n"
    }
    ...
    puts $chann "Arquivo com a especificação da pesquisa: "
    if {$export_html == 1} {
	puts $chann "</i>"
    }
    if {$export_html == 1} {
    ...
    #}
    #set num [llength $the_result]
    #puts $chann "Encontrado(s) $num íte(m/ns)\n"
    #if {$export_html == 1} {
	#puts $chann "</p>\n"
    #}
...
if {$argv == "--help"} {
  ...
  puts "Ferramenta de pesquisa por textos em arquivos."
  puts "Use: [string tolower $igl_prog_name] \[--version --license\] \[arquivo\]"
  puts "--license  licensa do programa"
  puts "--version  versão do programa"
  puts "arquivo	   arquivo (.flz) com especificação de pesquisa a ser carregado"
  set use_gui 0
} 
