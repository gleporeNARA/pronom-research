...
:i1.Introduction
:p.
OS/COM is a multi-port asynchronous serial device driver for the OS/2:link refid=IBM
reftype=fn.*:elink. operating system.  This version of OS/COM will work in any system
running OS/2 2.0 and above.  There is a different version for OS/2 1.x that is still
available and supported by OS/tools.
:p.
By multi-port we mean that with OS/COM there is no limit (logically) to the number of
...
that can be installed in any single system (server, client, workstation, CPU).  OS/COM
...
:p.
OS/COM comes with an easy to use installation and configuration program (INSTALL.EXE).  This
program can transfer all required files to the directories of your choice, create
a desktop folder and necessary program objects, and initiate the configuration process.
:p.
If you have multiple OS/COM licenses you can reuse a OS/COM initialization file for
similar installations by copying that initiaization file to the distribution
diskette.  When INSTALL finds a OS/COM initialization file in the directory it is run
from it will transfer that file to the installation destination directory.  The transferred
OS/COM initialization file will be presented for modification during each subsequent
installation process.
:p.
Multiple licenses for OS/COM can be installed from a network drive or
directory.  To do this you must first use INSTALL to transfer both OS/COM and INSTALL
to a network drive, then complete the configuration process just as though you were
installing it to a local drive.
:p.
Once installation and configuration is complete you can run INSTALL on a workstation
from the network directory to install OS/COM to that workstation.  Any initialization
...
selecting the :hp2.Config OS/COM...:ehp2. menu item after INSTALL has transferred
...
:p.
The OS/COM device driver can only be configured using Install.  Install will create,
or modify, an initialization file for the device driver.  Each time your system is
started, OS/COM reads this initialization file to determine where to look for
serial devices and how to configure those devices.
:p.
...
:p.
The OS/COM device driver will not operate in :link refid=ISA reftype=fn.ISA:elink.
...
program supplied with OS/COM.  After Installation is completed you will need to re-boot your
machine before you will be able to access serial devices with OS/COM.
:p.
After installation you can start the Install either from a program
object (icon) or from an OS/2:link refid=IBM reftype=fn.*:elink. command prompt.
:p.
:hp2.Starting from a program object&colon.:ehp2.
:p.
If there is a configuration program object you can double click
on the object to begin the installation/configuration process.
:p.
If there is no program object for Install, you can either start the
...
:lp.Example&colon.  C&colon.\COMM\INSTALL.EXE
...
:lp.Example&colon.  OS/COM Configuration
...
:p.
If the OS/COM device driver is loaded you will only need to enter the program name on
the command line.
:p.
Example&colon.  [C&colon.\]INSTALL <ENTER>
:p.
If the OS/COM device driver is not loaded you will be program to prompted for a path and
file name after starting Install.  The OS/COM initialization file
must be located in the same directory the device driver is located and must have
the same name as the drvice driver file with a ".INI" extension (SCOMDD.INI)..*
.**************************************************
.* Extensions (1)
.**************************************************
...
:i1.Extensions
:p.
OS/COM has extensions that allow it to perform in some special situations.  These
...
:p.
The OS/COM application interface is exactly as described in the "Physical Device
Driver" technical reference for Category One, Asynchronous Serial Device Drivers.
OS/COM supports all features and functions as described in that reference.
:p.
Differences from COM.SYS&colon.
:p.
There are some differences in how OS/COM is initialized compared to the COM.SYS device
driver.  Here is a list of the differences:
:ol.
:li.COM.SYS initializes at 1200 baud.  OS/COM defaults to 9600 baud.
:li.COM.SYS initializes to a protocol of seven data bits, even parity, and one stop
bit.  OS/COM defaults to a protocol of eight data bits, no parity, and one stop bit.
...
level of eight bytes. OS/COM defaults to hardware buffers on, with a receive trigger
level of fourteen bytes and a transmit buffer depth of sixteen bytes.
:li.COM.SYS has fixed receive and transmit buffers.  OS/COM's buffer sizes are user
definable.
:li.All COM.SYS defaults are fixed.  With OS/COM, all startup defaults, including
...
feature allows OS/COM to use startup defaults defined by the user.
:li.COM.SYS does not support COMscope, OS/COM does (this is important!).
:eol.
:p.
...
starts up with no handshaking enabled and neither does OS/COM, unless the end-user
makes it so.
.*
.**************************************************
...
:p.
OS/COM can work with any adapter that uses any of the required serial devices, in any
...
:p.
OS/COM is designed to allow an OS/2* system to access multiple serial devices at baud
...
including some built-in UARTs that are part of a motherboard chip set.
:p.
...
:eul.
:p.
 ISA shared interrupt capable adapters&colon.
:ol compact.
:li.:link refid=SUPPORT_SEA reftype=hd.Sealevel Systems:elink.
:eol.
:p.
...
:p.
The OS/COM device driver will automatically determine if any device it is configured to
...
reftype=fn.*:elink.  systems.  OS/COM will work with these UARTs but your system
...
:li.If you assign the same hardware interrupt level to two different adapters, or
devices, that do not support shared interrupts, you may lock-up your system,
especially when using higher baud rates and you are not accessing the port in a
separate thread from your window procedure.
:li.You may connect multiple, non-interrupt sharing, devices to a single interrupt
without problems if you open and access only one of those devices at a time.
:eol.
:p.
 Additional Information&colon.
:ol compact.
:li.:link refid=SUPPORT_SEA reftype=hd.Sealevel Systems Adapters:elink.
:eol.
.*
.**************************************************
.* Sealevel support (3)
...
:p.
When configuring OS/COM for a Sealevel adapter with shared interrupts you will
...
level.
:p.
More than one Sealevel adapter can be connected to a single hardware interrupt.
:p.
To set-up a Sealevel Systems eight port serial adpater to share interrupts you MUST
set all of the interrupt level select jumpers to the same interrupt.  If you do not
set all devices to the same interrupt your system WILL lock up if you access any device
that is not connected to the same interrupt that device number one is connected to.
:p.
The interrupt status/ID register address is the address of the first device plus seven.  If you
set the first device up at address 0x500 the interrupt status register will be located at
0x507.
.*
.**************************************************
.* Troubleshooting (1)
...
:p.
Believe it or not, the OS/COM installation and configuration process is not always as
...
when attempting to install and configure the OS/COM device driver&colon.
:ul compact.
...
:p.
Sealevel System systems serial adpaters that support interrupt sharing must be
configured using the :hp2.Adpter Set-up:ehp2. dialog.  Use this dialog to
select the interrupt level the adapter is configured for.
:p.
Neither COMscope nor CONFIG will normally allow you to configure more than one device
...
"wired-OR" interrupt circuit that ties each device to the system boards' Interrupt
...
device that is assigned to another OS/COM controlled device, nor will it allow you to
...
:p.
During each load of the OS/COM device driver each defined device is normally tested to
...
:p.
OS/COM will work when COM.SYS and VCOM.SYS are loaded.  If you want to use both OS/COM
and COM.SYS you will need to insure that COM.SYS is loaded before OS/COM.  The
configuration program will normally place OS/COM "DEVICE=" statements at the end of the
CONFIG.SYS file, so this should not be a problem.
:p.
You will also have to refrain from naming OS/COM devices with the same names COM.SYS
will use.  Naming a OS/COM device COM1 through COM4 will cause COM.SYS to drop access 
...
:p.
If you want to use COM.SYS for some COM ports and OS/COM for other's you need only
configure OS/COM to use the device name (COMx) you want COM.SYS to drop.
:p.
...
will not be able to use their respective COMx device names when configuring OS/COM.
:p.
...
:p.
Improperly configuring OS/COM and/or the serial adapter can cause various kinds of
...
control commands and their various input and output parameters.  OS/COM is designed to
operate according to that application interface.
:p.
Your distribution diskette contains a "C" source file that has sample code
...
thread, the time-out is set for the default one minute timeread time, and no
...
configuration of the device driver and/or serial adapter/device, and is the result of
an endless loop within the interrupt service routine while interrupts have been
disabled.  Make sure that the OS/COM device driver is configured correctly for your
adapter and that your adapter is configured as you intended.
.*
.**************************************************
.* Concepts (1)
.**************************************************
...
recommend that you leave APO off, aand FIFOs enabled, unless, and until, you determine
that you are communicating with some archaic equipment that requires it.
:fn id=IBM.
:p.
IBM, OS/2, PS/2, Micro Channel, and AT are all trade marks of International Business
...
:p.
The IBM PS/2 and compatables use a Micro Channel Architecture system bus.
:p.
IBM and PS/2 are trade marks of International Business Machines, Incorporated.
:efn.
:fn id=ISA.
:hp2.I:ehp2.ndustry :hp2.S:ehp2.tandard :hp2.A:ehp2.rchitecture
:p.
The IBM AT and compatables use an Industry Standard Architecture system bus.
:p.
IBM and AT are trade marks of International Business Machines, Incorporated.
:efn.
:euserdoc.
