...
#It is used to do backup of all files on this machine
#
DAY=`(set \`date\`;echo $1)`
...
echo "Subject: Tuesdays Backup Run Finished - $DAY $DATE $LMONTH $YEAR"
echo
...
#Load the Tuesdays tape to the drive (Load Slot 2 in Drive)
mtx -f /dev/sg2 load 2
...
echo "   <<< -------- WARNING !!!------- >>>"
echo
echo " Backed up to Tape in DLT Library Slot 2 (Tuesday), Tape for tomorrow will be Slot 3 !!! "
echo
...
rm end-date
rm start-date
#Unload Tuesdays Tape from the drive back to it's slot in the autoloader
mtx -f /dev/sg2 unload 2
