...
# Modification from original code includes minor changes and 'nt' os.
...
def receiveOnePing(mySocket,ID,timeout):
  ...
  while 1:
    ...
    if whatReady[0]==[]: # Timeout
      print "Timeout: ", mySocket, howLongInSelect, "\n"
      return None
...
def main():
  ...
  for ping in tries:
  	delay=doOne(dest)
  	print "Answere from\t", dest, "\tin ", delay, "second(s).\n"
	tdelay = tdelay + delay
  print "Average response time from", dest, ":\t", tdelay / 4.0," second(s)." 
  return None
