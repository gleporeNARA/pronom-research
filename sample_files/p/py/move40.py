...
# Current total score is sum of total numbers plus (50 minus total moves)
#There has to be incentive to get all values high, not just specific ones, score doesn't take this into account
...
def counters():
	...
	text = font.render("Score ", 1, (155,155,155))
	screen.blit(text, (665,10))
	text = font.render("Time ", 1, (155,155,155))
	screen.blit(text, (555,30))
	text = font.render("0", 1, (155,155,155))
	screen.blit(text, (735,10))
...
def add(card1_strength, card2_strength, card3_strength, card4_strength, card1_intelligence, card2_intelligence, card3_intelligence, card4_intelligence, card1_habitat, card2_habitat, card3_habitat, card4_habitat, card1_demeanor, card2_demeanor, card3_demeanor, card4_demeanor, card1_ability, card2_ability, card3_ability, card4_ability, card1_moved, card2_moved, card3_moved, card4_moved, card1_name, card2_name, card3_name, card4_name, card1_comp, card2_comp, card3_comp, card4_comp, card1_mat, card2_mat, card3_mat, card4_mat, card1_dev, card2_dev, card3_dev, card4_dev):
	...
	global total_score, total_strength, total_intelligence, total_habitat, total_demeanor, total_ability, total_comp, total_dev, total_mat
	...
	moves_madestr=str(int(moves_made))
	...
	font = pygame.font.Font(None, 24)
	text = font.render(moves_madestr, 1, (155,155,155))
	screen.blit(backgroundimg, [75, 40, 20, 20], [75, 40, 20, 20])
	screen.blit(text, (75, 40))
	font = pygame.font.Font(None, 18)
	...
	screen.blit(backgroundimg, [735, 10, 25, 15], [735, 10, 25, 15])
	screen.blit(text, (735,10))
...
def shuffledeal():
	...
	card16 = Card('bear2.png', 5, -1, -1, 0, 0, "Bear2", -5,0,0, 0)
        card17 = Card('deer2.png', 1, -5, -3, -1, -4, "Deer2", 0, 0,-10,0)
        card18 = Card('elephant2.png', 7, -1, 1, 4, 2, "Elephant2", 0, 0, -15, 0)
        card19 = Card('fox2.png', -1, -3, 5, -2, -4, "Fox2", 0, -10, 0, 0)
        card20 = Card('gazelle2.png', 2, 3, -8, -1, -3, "Gazelle2", -10,0,0, 0)
        card21 = Card('gorilla2.png', 10, 10, 10, 10, 10, "Gorilla2", 0, 0,0,0)
        card22 = Card('greyhound2.png', -1,-2, 6, 4, 1, "Greyhound2", 0, -5, 0, 0)
        card23 = Card('shark2.png', -10, -10, -10, -10, -10, "Shark2", -15, 0, 0, 0)
        card24 = Card('warthog2.png', -4, -7, -2, 4, 7, "Warthog2", 0,0,-10, 0)
        card25 = Card('wolf2.png', 3, -3, -8, -4, 2, "Wolf2", 0, -15,0,0)
        card26 = Card('snakehead2.png', -5,3, 10, 10, 7, "Snakehead2", -15, 0, 0, 0)
        card27 = Card('ostrich2.png', 4, 3, 8, 4, -7, "Ostrich2", 0, -15, 0, 0)
        card28 = Card('python2.png', 3,-5, -8, -2, -3, "Python2", -20,0,0, 0)
        card29 = Card('tiger2.png', 6, -3, 1, 4, 5, "Tiger2", 0, 0,-5,0)
        card30 = Card('coopershawk2.png', 4, 3, 1, 4, 7, "Coopers' Hawk2", 0, 0, -10, 0)
	card31 = Card('compresource.png', 0, 0, 0, 0, 0, "Resource", 10, 0, 0, 0)
	card32 = Card('matresource.png', 0, 0, 0, 0, 0, "Resource", 0, 10, 0, 0)
	card33 = Card('devresource.png', 0, 0, 0, 0, 0, "Resource", 0, 0, 10, 0)
	
	card_deck = [card1, card2, card3, card4, card5, card6, card7, card8, card9, card10, card11,\
...
def end_game(total_strength, total_intelligence, total_habitat, total_demeanor, total_ability):
	
	global total_score	
	total_score = (total_strength+total_intelligence+total_habitat+total_demeanor+total_ability)+(50-moves_made)
	total_scorestr=str(int(total_score))
	if total_score >10:
		...
		text = font.render("Total Score ", 1, (155,155,155))
		screen.blit(text, (335,250))
		text = font.render(total_scorestr, 1, (155,155,155))
		screen.blit(text, (395,280))
	else:
		font = pygame.font.Font(None, 96)
...
#Ok, ESC not pressed, check for keys...
	while 1:
		...
		if time <= 30:
			...
			screen.blit(backgroundimg, [595, 30, 25, 15], [595, 30, 25, 15])
			screen.blit(text, (595,30))
			pygame.display.update()	
			pygame.event.pump()
			...
			end_game(total_strength, total_intelligence, total_habitat, total_demeanor, total_ability)
			pygame.event.pump()
			keyinput = pygame.key.get_pressed()
			#event = pygame.event.get([KEYDOWN])
			if keyinput[K_ESCAPE] or pygame.event.peek(QUIT):
