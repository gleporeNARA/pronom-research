...
def identifyColumns(parsed):
    ...
    if columnLocations[0] == 'None':
        print ("ABORT NO NAME COLUMN FOUND")
    if columnLocations[3] == 'None' and columnLocations[4] == 'None' and columnLocations[5] == 'None':
        print ("ABORT NO Time COLUMN FOUND")
    ...
    #lnameMatch = re.compile('Smith|Johnson|Williams|Brown|Jones|Miller|Davis|Garcia|Rodriguez|Wilson', re.IGNORECASE)
    #timeMatch = re.compile('.*([0-5]?\d):([0-5]\d).*')
    #cityMatch = re.compile('Columbia|Ellicott City|Baltimore|Rockville|Silver Spring', re.IGNORECASE)
    #fname_values_count=0
    #lname_values_count=0
    #time_values_count=0
    #city_values_count=0
    ...
    #for column in df:
        ...
        #print ("Searching column " + str(column) + " for first name, last name, time or city matches.")
...
def findRecognizedCol(parsed):
    ...
    name_list = ['Name', 'First', 'Last', 'name', 'NAME', 'FIRST', 'LAST', 'Fname           Lname', 'Fname        Lname', 'Name     Last_name','First Name', 'Last Name', 'Fname', 'Lname']
    ...
    time_list = ['Net Time', 'NetTime', 'Finish', 'Time','Results','Net Tim']
    ...
    try:
        ...
        for gun_time_option in gun_time_list:
            ...
            if gun_time_option in parsed[0]:
                print ("found time \'" +  gun_time_option + "\' at location " + str(parsed[0].index(gun_time_option)))
        ...
        for chip_time_option in chip_time_list:
            ...
            if chip_time_option in parsed[0]:
                print ("found time \'" +  chip_time_option + "\' at location " + str(parsed[0].index(chip_time_option)))
